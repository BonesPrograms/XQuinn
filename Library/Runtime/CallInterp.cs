...
public class CallInterpreter
{
    ...
    //_.Field | load an instance field as the current instance.
    //_.Method() | return an instance from an instance method
    ...
    //selfnote: i would fix that easily by placing the number at the front (since you cant declare identifiers starting with numbers), but i plan to upgrade the lexical analyzer
    //to act more like a compiler and absorb context clues, knowing its reading an identifier if it sees a letter or _, knowing its reading a number if it sees a digit
    ...
    public object? Interface(string input, bool catchinvoke = false)
    {
        ...
        {
            if (input[1] != '.') //not super necessary, it really only exists to enforce a standard syntax. instance access must always be the same 2 chars: _.
                throw new ArgumentException($"Missing member access communicator for instance load. {input[0]}");
            return InstanceLoad(input.Substring(2));
        }
        else if (!catchinvoke)
            return Invoke(input);
        else
    ...
    object? StaticLoad(string accessor)
    {
        string[] inputs = accessor.Split('.');
        if (inputs.Length == 1)
            throw new ArgumentException($"static load input is invalid: {accessor}");
        Type type = FindType(new(inputs[0].Substring(0), null));
        object? instance;
        string invoc = inputs[inputs.Length - 1];
        int length = invoc.Length;
        if (length > 2 && (invoc[length - 1] == ')')) //method return
        ...
        if (instance == null)
            throw new ArgumentException($"Failed to load new instance from {accessor}");
        DirectLoadInstance(instance);
        return Instance;
    }
    ...
    protected virtual object? ParseParameter(string strng, Type type) //need half and int128 support
    {
        ...
        if (type == typeof(string))//&& strng.Length >= 2 && strng[0] == StringDeclr && strng[^1] == StringDeclr)
        {
            ...
            if (matches.Success)
                ...
                throw new FormatException("Non-null string values must be surrounded with quotations, even empty strings.");
        }
    ...
    static FieldInfo FindField(MemberMap tmap, Field field)
    {
        ...
        FieldInfo realfield = fields.FirstOrDefault(x => x.Name.EqualsCaseless(field.String)) as FieldInfo ?? throw new ArgumentException($"No field found named {field.String} in {field.DeclaringType}. It is possible it is an instance field, which are not available in static loads.");
        return realfield;
    }
