...
/// <summary>
/// RuntimeGateway is the unified chat-facing entry point for runtime navigation, registered commands, and dynamic invocation. It routes input according to a control key and returns the selected subsystem’s output.
/// </summary>
        ...
        {
                public readonly NavigationMonitor Monitor = new();
#if NET6_0_OR_GREATER
                public readonly DynamicAssembly? Assembly;
                public RuntimeGateway(string? assemblyPath = null, Func<Type, string?>? bookDelegate = null)
                {
                        if (assemblyPath != null && bookDelegate != null) Assembly = DynamicAssembly.New(assemblyPath, bookDelegate);
                }
#else
               ...
               , out string? dynInvokerOutput, out bool dynInvokerException
                ...
                )
                {
                        ...
                        if (invocation.Length > 0)
                                ...
                                if (invocation[0] == '%')
                                {
                                        if (Assembly == null) throw new InvalidOperationException("Cannot access dynamic invoker, it has not been initialized.");
                                        else return Assembly.Interface(invocation.Substring(1), out dynInvokerOutput, out dynInvokerException);
                                }
                                else
#endif
                                        if (invocation[0] == '/') { Command.InvokeCommand(invocation.Substring(1), Monitor.Interp, out object? ret); return ret; }
