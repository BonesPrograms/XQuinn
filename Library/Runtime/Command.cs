...
namespace XQuinn.Runtime
{
    ...
    {
        public CommandException(MethodInfo method, Command command, string msg) : base($"Method {method} in type {method.DeclaringType} with command name {command.Name} {msg}")
        {
    ...
    public sealed class HasCommand : Attribute
    {
    ...
    public sealed class Command : Attribute
    {
        ...
        /// </summary>
        public static bool InvokeCommand(string invocation, RuntimeNavigator interp, out object? ret)
        {
            ret = null;
        ...
        public static void Register(IEnumerable<Type> types)
        {
            foreach (Type type in types)
                if (type.GetCustomAttribute<HasCommand>() != null)
                    foreach (MethodInfo method in type.GetMethods(BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.DeclaredOnly | BindingFlags.Instance))
                    {
                        ...
                        if (command != null)
                        {
                            if (!method.IsStatic) throw new CommandException(method, command, "is not static.");
                            TypeCache.ThrowIfBadKey(command.Name);
                            if (!RuntimeNavigator.SupportedMember(method, method.GetParameters())) throw new CommandException(method, command, "has an unsupported in out or ref parameter, or ref return type.");
                            if (_registry.TryGetValue(command.Name, out MethodInfo? alreadyCached)) { if (method != alreadyCached) throw new CommandException(method, command, "cannot be cached, command name is already taken."); else continue; }
